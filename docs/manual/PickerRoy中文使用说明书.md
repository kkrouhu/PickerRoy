...
版本 0.2.0 · Mac 与 Windows · 2026 年 9 月
...
7. 评估清晰度、曝光、对比、视觉平衡、三分构图、运动与时序峰值。
8. 用本地热门视觉模型给有效候选排序。
...
这些决定保存在本机数据库里。单次“保留/淘汰/收藏”和 A/B 偏好是两种记录：前者表达结果处置，后者专门用于训练相对审美。
...
“偏好训练”会提供同一镜头、相近时刻的 A/B 对比。选择你更喜欢的一张，或跳过难以判断的一组。

每次选择会保存胜者和败者的技术、构图、内容、时序与热门视觉特征。下一次分析时，PickerRoy 在本机训练一个正则化 Bradley–Terry 成对排序模型。

为避免一两次误点影响太大，个人偏好权重会随样本数量逐步增加，初期约 6%，上限约 16%。建议：
...
当前版本没有抓取你的社交媒体，也没有后台收集图片。用户自己的训练数据只来自本机 A/B 选择。
