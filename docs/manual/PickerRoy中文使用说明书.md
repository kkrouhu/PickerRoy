...
在 iPhone/iPad 上，视频通过系统文件选择器导入；导出时 PickerRoy 只申请“添加照片”权限，把所选结果写入系统相册，不读取整个图库。视频分析、个性化与图片生成没有网络依赖；真机飞行模式完整流程仍是正式上架前的必测项。
...
3. 双击打开。
4. 当前社区构建没有使用 Roy 自己的 Apple Developer ID 公证，若首次双击被系统拦截，请按住 Control 点按 PickerRoy，选择“打开”，再确认一次“打开”。不要全局关闭 Gatekeeper。
